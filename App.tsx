...
function App() {
  ...
  const startQuiz = async (category: Category, subTopic: string | null = null) => {
    audioService.playClick();
    setLoading(true);
    setLoadingPhase("INITIALIZING LONG-RANGE MISSION...");
    ...
    try {
      ...
      const qs = await generateQuestions(category, subTopic, count);
      
      setLoadingPhase("DEPLOYING CINEMATIC INTEL ASSETS...");
      
      const q1 = shuffleArray(qs).map(q => ({
        ...q,
        options: shuffleArray(q.options)
      }));
      const q2 = shuffleArray(qs).map(q => ({
        ...q,
        options: shuffleArray(q.options)
      }));
      ...
      alert("Gagal memuat misi. Coba lagi.");
    } finally {
      setLoading(false);
      setLoadingPhase("");
    }
  ...
  const handleAnswerP1 = (isCorrect: boolean) => {
    ...
    if (indexP1 + 1 < questionsP1.length) {
      setTimeout(() => setIndexP1(i => i + 1), 600);
    } else {
      setFinishedP1(true);
    }
  };
  ...
  const handleAnswerP2 = (isCorrect: boolean) => {
    ...
    if (indexP2 + 1 < questionsP2.length) {
      setTimeout(() => setIndexP2(i => i + 1), 600);
    } else {
      setFinishedP2(true);
    }
  };
  ...
  const renderScreen = () => {
    switch (screen) {
      ...
      case GameScreen.INSTRUCTIONS:
        return <InstructionsScreen onNext={() => setScreen(GameScreen.CHAR_SELECT)} />;
      ...
      case GameScreen.GAMEPLAY:
        return (
          <SplitGameplayScreen 
            questionsP1={questionsP1}
            questionsP2={questionsP2}
            indexP1={indexP1}
            indexP2={indexP2}
            scoreP1={scoreP1}
            scoreP2={scoreP2}
            finishedP1={finishedP1}
            finishedP2={finishedP2}
            onAnswerP1={handleAnswerP1}
            onAnswerP2={handleAnswerP2}
          />
        );
      case GameScreen.RESULT:
        return (
          <ResultScreen 
            scoreP1={scoreP1}
            scoreP2={scoreP2} 
            total={questionsP1.length * 5} 
            onHome={() => setScreen(GameScreen.MAP)} 
          />
        );
      default:
        return null;
    ...
    <div className="relative w-screen h-screen bg-black overflow-hidden select-none">
      ...
      {loading && (
        <div className="absolute inset-0 z-50 bg-black/95 flex flex-col items-center justify-center font-ops text-yellow-500 tracking-widest text-center p-4">
          ...
          <h2 className="text-2xl mb-2">MATCHMAKING...</h2>
          <div className="flex flex-col items-center max-w-sm">
            <p className="text-sm text-white/70 uppercase font-mono tracking-tighter">{loadingPhase}</p>
            <p className="text-[10px] text-yellow-500/50 mt-2 italic">Generating 20 high-quality visuals may take up to 2 minutes.</p>
            <div className="mt-6 w-64 h-1 bg-white/10 rounded-full overflow-hidden">
                <div className="h-full bg-yellow-500 animate-[loading-bar_3s_infinite]"></div>
            </div>
          </div>
          <style>{`
                ...
                {['Penjumlahan', 'Pengurangan', 'Perkalian', 'Pembagian'].map(op => (
                  <button 
                    ...
                    onClick={() => {
                      ...
                      startQuiz(Category.MATH, op);
                    }}
                    className="bg-slate-800 hover:bg-cyan-600 border border-slate-600 hover:border-white p-4 text-white font-bold transition-all clip-diagonal hover:scale-105"
                  >
                    {op.toUpperCase()}
                  </button>
                ))}
...
const TitleScreen = ({ onStart }: { onStart: () => void }) => (
  <div className="w-full h-full flex flex-col items-center justify-center bg-[url('https://picsum.photos/1920/1080?grayscale&blur=2')] bg-cover bg-center">
    ...
    <button 
      onClick={onStart}
      className="z-10 mt-16 px-12 py-4 bg-yellow-600 hover:bg-yellow-500 text-black font-ops text-2xl clip-diagonal transform hover:scale-110 transition-transform duration-200 shadow-[0_0_20px_rgba(234,179,8,0.5)]"
    >
      TAP TO START
    </button>
  </div>
...
const LoginScreen = ({ onLogin }: { onLogin: (n: string, g: string) => void }) => {
  ...
  const handleType = (e: React.ChangeEvent<HTMLInputElement>, setter: React.Dispatch<React.SetStateAction<string>>) => {
    audioService.playType();
        ...
        <div className="space-y-6">
          <div>
            ...
            <input 
              type="text" 
              value={name}
              onChange={(e) => handleType(e, setName)}
              className="w-full bg-black/50 border border-slate-600 text-white p-3 focus:border-yellow-500 focus:outline-none font-mono"
          ...
          <div>
            ...
            <input 
              type="text" 
              value={grade}
              onChange={(e) => handleType(e, setGrade)}
              className="w-full bg-black/50 border border-slate-600 text-white p-3 focus:border-yellow-500 focus:outline-none font-mono"
              placeholder="Enter Class..."
            />
          </div>
          <button 
            onClick={() => { if(name && grade) onLogin(name, grade); }}
            disabled={!name || !grade}
            className="w-full py-4 mt-4 bg-yellow-600 hover:bg-yellow-500 disabled:bg-slate-600 text-black font-ops text-xl clip-diagonal transition-colors"
          >
            CONFIRM IDENTITY
          </button>
        </div>
...
const InstructionsScreen = ({ onNext }: { onNext: () => void }) => (
  <div className="w-full h-full flex flex-col items-center justify-center bg-slate-950 p-8">
    <div className="max-w-3xl w-full border border-yellow-500/30 bg-slate-900/90 p-8 relative">
      <h2 className="font-ops text-3xl text-yellow-500 mb-6 text-center uppercase tracking-tighter">BATTLE MODE: 20 ROUNDS</h2>
      <ul className="space-y-4 text-slate-300 font-mono text-lg list-disc pl-6">
        <li>Layar akan terbagi dua (Kiri & Kanan).</li>
        <li>Soal dilengkapi dengan TACTICAL INTEL (Gambar Soal).</li>
        <li>Tantangan maraton: Selesaikan 20 soal berturut-turut.</li>
        <li>Pilih jawaban yang paling tepat dari atas ke bawah.</li>
        <li>Dapatkan BOOYAH! setelah membersihkan semua sektor.</li>
      </ul>
      <button 
        onClick={() => { audioService.playClick(); onNext(); }}
        className="mt-8 float-right px-8 py-2 bg-yellow-600 text-black font-bold font-ops clip-diagonal hover:bg-white hover:text-black transition-all"
      >
        START BATTLE
      </button>
    </div>
    ...
    <div className="flex-1 overflow-x-auto flex items-center gap-8 px-4 pb-4">
      {CHARACTERS.map(char => (
        <div 
          ...
          className="min-w-[280px] h-[450px] bg-slate-800 border-2 border-slate-600 hover:border-yellow-500 relative cursor-none group transition-all transform hover:-translate-y-4"
        >
      ...
      <div className="absolute inset-0 flex items-center justify-center p-8 pt-28 overflow-y-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 w-full max-w-7xl pb-10">
          {Object.values(Category).map((cat, idx) => {
            const score = user?.scores[cat] || user?.scores[cat + ' (Penjumlahan)'] || 0; 
            const style = getCategoryColor(cat);
            return (
              <button 
                key={idx}
                onClick={() => onSelectCategory(cat)}
                className={`relative h-40 bg-gradient-to-br border-2 transition-all duration-300 group clip-diagonal text-left p-0 overflow-hidden hover:scale-105 hover:z-10 shadow-lg ${style}`}
              >
                ...
                <div className="p-5 h-full flex flex-col justify-between relative z-10">
                    <div className="flex justify-between items-start">
                        <div className="p-2 bg-black/30 rounded border border-white/10 backdrop-blur-sm">
                            {getIcon(cat)}
                        </div>
  ...
  if (finished) {
    return (
      <div className={`w-1/2 h-full flex flex-col items-center justify-center bg-black/80 border-${isMirrored ? 'l' : 'r'} border-white/10 p-4 text-center`}>
        <h2 className={`font-ops text-5xl mb-4 ${colorClass}`}>MISSION COMPLETE</h2>
        <div className="text-8xl font-mono text-white mb-2">{score}</div>
        <div className="text-lg text-slate-500 tracking-widest uppercase">Points Secured</div>
      </div>
    );
  }

  if (!currentQ) return <div className="w-1/2"></div>;
