...
// Advanced synthesizer for Free Fire style dynamic BGM
class AudioService {
  ...
  playTheme(screen: GameScreen) {
    ...
    this.resume(); // Try to resume if allowed

    // Set Tempo based on screen
    switch(screen) {
        case GameScreen.TITLE:
        case GameScreen.LOGIN:
            this.tempo = 60; // Slow, suspense
            break;
        case GameScreen.INSTRUCTIONS:
        case GameScreen.CHAR_SELECT:
        case GameScreen.MAP:
            this.tempo = 105; // Lobby groove
            break;
        case GameScreen.GAMEPLAY:
            this.tempo = 128; // Fast action
            break;
        case GameScreen.RESULT:
            this.tempo = 90; // Celebration
            break;
        default:
            this.tempo = 100;
    }
  ...
  // 1. Suspense (Deep heartbeat)
  private scheduleSuspense(beat: number, time: number) {
      // Heartbeat on 0 and 10 (syncopated feel)
      if (beat === 0 || beat === 3) {
          this.playKick(time, 0.3, 60);
      }
      // Low drone pulse
      if (beat === 0) {
          this.playBass(time, 0.1, 40, 0.5);
      }
  }

  // 2. Lobby (Tactical Breakbeat)
  ...
  private scheduleBattle(beat: number, time: number) {
      ...
      // Driving Bass (16ths)
      const bassNote = beat < 8 ? 55 : 65; // A -> C#
